          ...
          <ResponsiveContainer width="100%" height={400}>
            {activeTab === 'growth' ? (
              <LineChart data={projectionData}>
                ...
                <YAxis stroke="#ffffff80" tickFormatter={(val) => `${getCurrencySymbol(portfolio.baseCurrency)}${(val / 1000).toFixed(0)}k`} />
                <Tooltip 
                  contentStyle={{ backgroundColor: '#1e1b4b', border: '1px solid #ffffff30', borderRadius: '8px' }}
                  labelStyle={{ color: '#ffffff' }}
                  formatter={(value) => [`${getCurrencySymbol(portfolio.baseCurrency)}${value.toLocaleString()}`, '']}
                />
                <Legend />
                <Line 
                  type="monotone" 
                ...
                <Line 
                  ...
                  connectNulls
                />
                {showIndividualAccounts && portfolio.accounts.filter(acc => selectedAccounts[acc.id]).map((account, idx) => (
                  <Line
                    key={account.id}
                    type="monotone"
                    dataKey={`account_${account.id}`}
                    name={account.name}
                    stroke={`hsl(${(idx * 360) / portfolio.accounts.length}, 70%, 60%)`}
                    strokeWidth={2}
                    dot={false}
                    animationDuration={1500}
                  />
              ...
              <BarChart data={projectionData}>
                ...
                <Tooltip 
                  contentStyle={{ backgroundColor: '#1e1b4b', border: '1px solid #ffffff30', borderRadius: '8px' }}
                  labelStyle={{ color: '#ffffff' }}
                  formatter={(value, name) => [`${getCurrencySymbol(portfolio.baseCurrency)}${value?.toLocaleString() || '0'}`, name]}
                />
                <Legend />
                {showIndividualAccounts ? (
                  portfolio.accounts.filter(acc => selectedAccounts[acc.id]).map((account, idx) => (
                    <Bar
                      key={account.id}
                      dataKey={`account_${account.id}_yield`}
                      name={account.name}
                      fill={`hsl(${(idx * 360) / portfolio.accounts.length}, 70%, 60%)`}
                      animationDuration={1500}
                    />
                  ))
                ) : (
